...
public class InternetConnectionCheckerBinary {
    ...
    public static void main(String[] args) {
        // If user invoked with `readlog` command, print the contents of the binary log
        // and exit.
        ...
        try (DataOutputStream logStream = new DataOutputStream(new FileOutputStream(LOG_FILE))) {
            while (true) {
                ...
                System.out.printf("[%tF %<tT] Status: %s%n", timestamp, isConnected ? "CONNECTED" : "DISCONNECTED");
    ...
    /**
     * Read and print the entire binary log file produced by this program.
     * Each record is a long (8 bytes) timestamp followed by a boolean (1 byte).
     */
    ...
    /**
     * Read and print last `n` records from the binary log file. If n is greater
     * than
     * available records, the whole file is printed.
     */
    public static void readLastNLog(String path, int n) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(f, "r")) {
            ...
            for (long i = 0; i < toRead; i++) {
                ...
                System.out.printf("[%1$tF %1$tT] Status: %s%n", ts, status ? "CONNECTED" : "DISCONNECTED");
            }
        }
    }
}
